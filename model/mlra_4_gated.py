...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        kv = torch.einsum('btld,ldo->btlo', compressed_kv, self.kv_b_proj)  # [B, T, n_latent_head, n_head * 2 * qk_nope_head_dim]
        kv = kv.view(B, T, self.n_latent_head, self.n_head, 2 * self.qk_nope_head_dim)
        
        k_nope_branches = []
        v_branches = []
        for i in range(self.n_latent_head):
            kv_branch = kv[:, :, i, :, :]  # [B, T, n_head, 2 * qk_nope_head_dim]
            k_nope, v = torch.split(kv_branch, [self.qk_nope_head_dim, self.qk_nope_head_dim], dim=-1)
            k_nope_branches.append(k_nope)  # [B, T, n_head, qk_nope_head_dim]
            v_branches.append(v)            # [B, T, n_head, qk_nope_head_dim]
        
        # ==================== Apply RoPE ====================
        cos_q, sin_q = self.rotary(q_pe)
        cos_k, sin_k = self.rotary(k_pe)
        q_pe = apply_rotary_emb(q_pe, cos_q, sin_q)  # [B, T, n_head, qk_rope_head_dim]
        k_pe = apply_rotary_emb(k_pe, cos_k, sin_k)  # [B, T, 1, qk_rope_head_dim]
        ...
        y = 0
        for i in range(self.n_latent_head):
            k_i = k_pe.new_empty(B, T, self.n_head, self.qk_head_dim)
            k_i[:, :, :, :self.qk_nope_head_dim] = k_nope_branches[i]
            k_i[:, :, :, self.qk_nope_head_dim:] = k_pe  # shared across all branches
            
            y_i = flash_attn_func(q=q_new, k=k_i, v=v_branches[i], causal=True) * self.attn_output_scale
            y = y + y_i  # Sum across branches
