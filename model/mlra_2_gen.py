...
class CausalSelfAttention(nn.Module):
    ...
    def _precompute_absorption(self):
        # kv_b_proj.weight: [n_latent_head, kv_latent_dim, n_group_head * 2 * qk_nope_head_dim]
        w = self.kv_b_proj.view(
            self.n_latent_head, self.kv_latent_dim, self.n_group_head, 2 * self.qk_nope_head_dim
        ).permute(0, 2, 3, 1).contiguous()

        # W_UK: [n_latent_head, n_group_head, qk_nope_head_dim, kv_latent_dim]
        self.W_UK = w[:, :, :self.qk_nope_head_dim, :].contiguous()
        # W_UV: [n_latent_head, n_group_head, kv_latent_dim, qk_nope_head_dim]
    ...
    def forward(self, x):
        ...
        q = self.q_b_proj(compressed_q).view(B, T, self.n_head, self.qk_head_dim)
        ...
        head_slices = [
            slice(0, self.n_group_head),
            slice(self.n_group_head, self.n_head),
            slice(0, self.n_group_head),
            slice(self.n_group_head, self.n_head),
        ]
        ...
        if is_prefill:
            ...
            for i in range(self.n_latent_head):
                ...
                q_pe_i = q_pe[:, :, hs, :]
                q_nope_i = q_nope[:, :, hs, :]
                q_absorbed_i = torch.einsum("bthp,hpq->bthq", q_nope_i, self.W_UK[i])
                z_i = flash_attn_func(
                    q=q_pe_i, k=k_pe, v=normed_kv_latents[i], qv=q_absorbed_i, causal=True
                )
                y_i = torch.einsum("bthq,hqp->bthp", z_i, self.W_UV[i]) * self.attn_output_scale
                y_group.append(y_i)

            y_a = torch.cat([y_group[0], y_group[1]], dim=2)
            y_b = torch.cat([y_group[2], y_group[3]], dim=2)
            y = y_a + y_b
