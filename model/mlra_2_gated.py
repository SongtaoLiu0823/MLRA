...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.n_group_head = self.n_head // 2
        self.qk_rope_head_dim = config.qk_rope_head_dim
        self.qk_nope_head_dim = config.qk_nope_head_dim
        self.qk_head_dim = self.qk_nope_head_dim + self.qk_rope_head_dim

        self.gated_proj = nn.Linear(self.n_embd, self.qk_nope_head_dim * self.n_head, bias=False)
        ...
        self.kv_b_proj = nn.Parameter(
            torch.empty(self.n_latent_head, self.kv_latent_dim, self.n_group_head * 2 * self.qk_nope_head_dim)
        )
        self.kv_scale = (self.n_embd / self.kv_latent_dim) ** 0.5
        self.attn_output_scale = 1 / (2 ** 0.5)
    ...
    def reset_parameters(self):
        nn.init.normal_(self.gated_proj.weight, mean=0.0, std=0.02)
    ...
    def forward(self, x):
        ...
        gated_score = F.sigmoid(self.gated_proj(x))
        # ==================== Process Q ====================
        ...
        for i, latent in enumerate(kv_latents):
            normed_kv_latents.append(self.kv_a_layernorms[i](latent) * self.kv_scale)
        compressed_kv = torch.stack(normed_kv_latents, dim=2)  # [B, T, n_latent_head, kv_latent_dim]
        kv = torch.einsum('btld,ldo->btlo', compressed_kv, self.kv_b_proj)  # [B, T, n_latent_head, n_group_head * 2 * qk_nope_head_dim]
        kv = kv.view(B, T, self.n_latent_head, self.n_group_head, 2 * self.qk_nope_head_dim)
        
        # Branch A: latent head 0, 1
        kv_branch_a = kv[:, :, :2, :, :].reshape(B, T, self.n_head, 2 * self.qk_nope_head_dim)
        # Branch B: latent head 2, 3
        kv_branch_b = kv[:, :, 2:, :, :].reshape(B, T, self.n_head, 2 * self.qk_nope_head_dim)
        
        k_nope_a, v_a = torch.split(kv_branch_a, [self.qk_nope_head_dim, self.qk_nope_head_dim], dim=-1)
        k_nope_b, v_b = torch.split(kv_branch_b, [self.qk_nope_head_dim, self.qk_nope_head_dim], dim=-1)
        ...
        # ==================== Assemble Q and K ====================
        q_new = q_pe.new_empty(B, T, self.n_head, self.qk_head_dim)
        q_new[:, :, :, : self.qk_nope_head_dim] = q_nope
        q_new[:, :, :, self.qk_nope_head_dim :] = q_pe
        k_a = k_pe.new_empty(B, T, self.n_head, self.qk_head_dim)
        k_a[:, :, :, : self.qk_nope_head_dim] = k_nope_a
        k_a[:, :, :, self.qk_nope_head_dim :] = k_pe
        k_b = k_pe.new_empty(B, T, self.n_head, self.qk_head_dim)
        k_b[:, :, :, : self.qk_nope_head_dim] = k_nope_b
        k_b[:, :, :, self.qk_nope_head_dim :] = k_pe
        
        y_a = flash_attn_func(q=q_new, k=k_a, v=v_a, causal=True) * self.attn_output_scale
        y_b = flash_attn_func(q=q_new, k=k_b, v=v_b, causal=True) * self.attn_output_scale
        y = y_a + y_b
