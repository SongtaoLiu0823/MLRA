...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.kv_a_layernorm_1 = RMSNorm(self.kv_lora_rank // 2)
        self.kv_a_layernorm_2 = RMSNorm(self.kv_lora_rank // 2)
        self.kv_b_proj_1 = nn.Linear(self.kv_lora_rank // 2, self.n_head * self.qk_nope_head_dim, bias=False)
        self.kv_b_proj_2 = nn.Linear(self.kv_lora_rank // 2, self.n_head * self.qk_nope_head_dim, bias=False)
        self.kv_scale = (self.n_embd / (self.kv_lora_rank // 2)) ** 0.5
    ...
    def reset_parameters(self):
        ...
        nn.init.normal_(self.kv_b_proj_1.weight, mean=0.0, std=0.02)
        nn.init.normal_(self.kv_b_proj_2.weight, mean=0.0, std=0.02)
    ...
    def forward(self, x):
        ...
        compressed_kv_1, compressed_kv_2, k_pe = torch.split(all_kv, [self.kv_lora_rank // 2, self.kv_lora_rank // 2, self.qk_rope_head_dim], dim=-1)
        k_pe = k_pe.view(B, T, 1, self.qk_rope_head_dim)
        
        compressed_kv_1 = self.kv_a_layernorm_1(compressed_kv_1) * self.kv_scale
        kv_1 = self.kv_b_proj_1(compressed_kv_1).view(B, T, self.n_head // 2, 2 * self.qk_nope_head_dim)
        k_nope_1, v_1 = torch.split(kv_1, [self.qk_nope_head_dim, self.qk_nope_head_dim], dim=-1)

        compressed_kv_2 = self.kv_a_layernorm_2(compressed_kv_2) * self.kv_scale
        kv_2 = self.kv_b_proj_2(compressed_kv_2).view(B, T, self.n_head // 2, 2 * self.qk_nope_head_dim)
        k_nope_2, v_2 = torch.split(kv_2, [self.qk_nope_head_dim, self.qk_nope_head_dim], dim=-1)
        ...
        k[:, :, : self.n_head // 2, : self.qk_nope_head_dim] = k_nope_1
        k[:, :, self.n_head // 2 :, : self.qk_nope_head_dim] = k_nope_2
        k[:, :, :, self.qk_nope_head_dim :] = k_pe
        v = torch.cat([v_1, v_2], dim=2)
