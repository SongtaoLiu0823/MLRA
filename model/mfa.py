...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.share_q_dim = config.share_q_dim

        self.c_q = nn.Linear(self.n_embd, self.share_q_dim, bias=False)
        self.q_layernorm = RMSNorm(self.share_q_dim)
        self.w_q = nn.Linear(self.share_q_dim, self.head_dim * self.n_head, bias=False)
        self.c_k = nn.Linear(self.n_embd, self.head_dim, bias=False)
        self.c_v = nn.Linear(self.n_embd, self.head_dim, bias=False)
    ...
    def forward(self, x):
        ...
        normed_q = self.q_layernorm(self.c_q(x))
        q = self.w_q(normed_q).view(B, T, self.n_head, self.head_dim)
        k = self.c_k(x).view(B, T, 1, self.head_dim)
        v = self.c_v(x).view(B, T, 1, self.head_dim)
        cos, sin = self.rotary(q)
        q, k = apply_rotary_emb(q, cos, sin), apply_rotary_emb(k, cos, sin)
        
        k = torch.repeat_interleave(k, self.n_head, dim=2)
        v = torch.repeat_interleave(v, self.n_head, dim=2)
