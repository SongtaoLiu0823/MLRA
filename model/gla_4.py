...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        assert config.n_head % self.n_latent_head == 0
        self.kv_latent_dim = self.kv_lora_rank // self.n_latent_head
        self.n_group_head = self.n_head // self.n_latent_head
        ...
        self.kv_a_layernorms = nn.ModuleList(
            [RMSNorm(self.kv_latent_dim) for _ in range(self.n_latent_head)]
        )
    ...
    def forward(self, x):
        ...
        compressed_kv_latents = torch.split(compressed_kv, self.kv_latent_dim, dim=-1)
        normed_kv_latents = []
        for i, compressed_latent in enumerate(compressed_kv_latents):
            normed_kv_latents.append(self.kv_a_layernorms[i](compressed_latent) * self.kv_scale)        
        compressed_kv = torch.cat(normed_kv_latents, dim=-1) # Shape [B, T, 512]
        compressed_kv = compressed_kv.view(B, T, self.n_latent_head, self.kv_latent_dim)
        kv = torch.einsum('btlh,lhd->btld', compressed_kv, self.kv_b_proj)
        
        # Reshape to separate the heads produced by each latent head
        kv = kv.view(B, T, self.n_latent_head, self.n_group_head, 2 * self.qk_nope_head_dim)
        
        # Flatten to get all output heads
        kv = kv.reshape(B, T, self.n_head, 2 * self.qk_nope_head_dim)
